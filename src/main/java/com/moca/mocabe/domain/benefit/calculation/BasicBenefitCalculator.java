...
public class BasicBenefitCalculator implements BenefitCalculator {
  ...
  public BenefitCalculationResult calculate(BenefitRule rule, BenefitCalculationContext context) {
    ...
    return new BenefitCalculationResult(
        ...
        rawRewardValue,
        appliedRewardValue,
        remainingLimitAfter(rule, appliedRewardValue),
  ...
  private BigDecimal calculateRawReward(BenefitRule rule, BenefitCalculationContext context) {
    return switch (rule.benefitBasis()) {
      case RATE ->
          multiplyAndFloor(benefitBaseAmount(rule, context.paymentAmount()), rule.rewardRate());
      case FIXED -> rule.rewardValue();
      // 12,900원에 1,000원당 1포인트라면 12포인트만 적립한다.
      case PER_SPEND_UNIT ->
          isZeroOrNegative(rule.spendUnitAmount())
              ? BigDecimal.ZERO
              : multiplyAndFloor(
                  context.paymentAmount().divideToIntegralValue(rule.spendUnitAmount()),
                  rule.rewardValue());
      case PER_USAGE_UNIT ->
          multiplyAndFloor(benefitUsageQuantity(rule, context), rule.rewardValue());
    };
  }
  ...
  private BigDecimal benefitUsageQuantity(BenefitRule rule, BenefitCalculationContext context) {
    if (isZeroOrNegative(rule.maximumBenefitBaseAmount())
        || isZeroOrNegative(context.paymentAmount())) {
  ...
  private BigDecimal multiplyAndFloor(BigDecimal value, BigDecimal multiplier) {
    ...
    return value.multiply(multiplier).setScale(0, RoundingMode.DOWN);
  }

  private BigDecimal capPaymentAmount(
      BenefitRule rule, BigDecimal rawRewardValue, BigDecimal paymentAmount) {
  ...
  private BigDecimal applyMonthlyLimit(BenefitRule rule, BigDecimal rawRewardValue) {
    ...
    BigDecimal remainingLimit =
        max(rule.monthlyLimitValue().subtract(rule.usedMonthlyValue()), BigDecimal.ZERO);
  ...
  private BigDecimal remainingLimitAfter(BenefitRule rule, BigDecimal appliedRewardValue) {
    ...
    return max(
        rule.monthlyLimitValue().subtract(rule.usedMonthlyValue()).subtract(appliedRewardValue),
        BigDecimal.ZERO);
  }
