...
public record BenefitCalculationContext(
    ...
    public BenefitCalculationContext(BigDecimal paymentAmount, BigDecimal usageQuantity,
            BigDecimal previousMonthSpend, LocalDateTime approvedAt, String mocaCategory,
            boolean newMemberGracePeriod, int usedDailyCount, int usedMonthlyCount,
            boolean merchantEligible, boolean paymentChannelEligible) {
        this(paymentAmount, usageQuantity, previousMonthSpend, approvedAt, mocaCategory, newMemberGracePeriod,
                usedDailyCount, usedMonthlyCount, merchantEligible, paymentChannelEligible, false, Map.of());
    }
    ...
    public boolean hasTarget(String targetType, String targetCode) {
        ...
        if ("MERCHANT_CATEGORY".equals(normalizedType) && normalizedCode.equals(normalize(mocaCategory))) {
            return true;
        }
    ...
    private static Map<String, Set<String>> normalizeAttributes(Map<String, Set<String>> attributes) {
        ...
        attributes.forEach((type, values) -> normalized.put(
                normalize(type),
                values == null ? Set.of() : values.stream().map(BenefitCalculationContext::normalize)
