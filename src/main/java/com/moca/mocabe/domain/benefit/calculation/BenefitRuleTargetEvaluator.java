...
/**
 * 같은 condition_group의 include는 AND, 서로 다른 그룹은 OR로 평가한다.
 * exclude는 그룹과 무관하게 하나라도 일치하면 최종 대상에서 제외한다.
 */
public class BenefitRuleTargetEvaluator {
    ...
    public boolean matches(Set<BenefitRuleTarget> targets, BenefitCalculationContext context) {
        ...
        boolean excluded = targets.stream()
        ...
        Map<Integer, List<BenefitRuleTarget>> includeGroups = targets.stream()
        ...
        return includeGroups.isEmpty() || includeGroups.values().stream()
