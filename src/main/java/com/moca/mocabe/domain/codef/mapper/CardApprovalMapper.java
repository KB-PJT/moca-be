...
public interface CardApprovalMapper {
    ...
    /**
     * 카드번호가 필요한 카드사에서 연동(codefAccountCredentialId) 하나에 속한 활성 카드별
     * 카드번호/비밀번호를 조회한다. 승인내역·실적조회를 카드마다 개별 호출하는 데 쓰인다.
     */
    List<ActiveCardCredential> findActiveCardCredentialsByCredentialId(
            @Param("codefAccountCredentialId") String codefAccountCredentialId);
