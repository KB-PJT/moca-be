...
public class AppConfig {
  ...
  public BenefitHistoryQueryService benefitHistoryQueryService(
      BenefitHistoryMapper benefitHistoryMapper) {
  ...
  public UserApplicationService userApplicationService(
      UserDomainService userDomainService, OpaqueTokenService opaqueTokenService) {
  ...
  public Encryptor codefEncryptor(Environment environment) {
    byte[] key =
        Base64.getDecoder().decode(requiredProperty(environment, "MOCA_CREDENTIAL_ENCRYPTION_KEY"));
  ...
  public CredentialHasher credentialHasher(Environment environment) {
    byte[] key =
        Base64.getDecoder().decode(requiredProperty(environment, "MOCA_CREDENTIAL_HASH_KEY"));
  ...
  public CodefHttpClient codefHttpClient(HttpClient codefJavaHttpClient, Environment environment) {
    return new JdkCodefHttpClient(
        codefJavaHttpClient,
        timeoutProperty(
            environment, "MOCA_CODEF_REQUEST_TIMEOUT_MS", DEFAULT_CODEF_REQUEST_TIMEOUT_MS));
  }
  ...
  @Bean
  public MerchantLookup merchantLookup(
      MerchantMapper merchantMapper, MerchantNameNormalizer merchantNameNormalizer) {
  ...
  @Bean
  public PerformanceSnapshotStore performanceSnapshotStore(
      CardPerformanceMapper cardPerformanceMapper) {
  ...
  /**
   * CODEF 호출 URL(baseUrl/tokenUrl) 환경변수를 https + 승인된 CODEF host로 제한한다. Bearer 토큰·Basic 자격증명이 설정 실수로
   * 외부 endpoint에 전송되는 것을 막기 위한 방어다.
   */
  private static String requiredCodefUrl(Environment environment, String name) {
    ...
    if (host == null || !ALLOWED_CODEF_HOSTS.contains(host.toLowerCase(Locale.ROOT))) {
      throw new IllegalStateException(name + "의 host가 승인된 CODEF host가 아닙니다: " + host);
    }
    return value;
  }
