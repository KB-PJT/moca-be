...
class GlobalExceptionHandlerTest {
    ...
    ResponseEntity<ApiErrorResponse> response =
        handler.handleDataAccess(
            new CannotGetJdbcConnectionException(
                "password should not be exposed", new SQLException()));

    assertEquals(HttpStatus.SERVICE_UNAVAILABLE, response.getStatusCode());
    assertEquals("DATA_STORE_UNAVAILABLE", response.getBody().getError().getCode());
    assertEquals("데이터 저장소에 일시적으로 연결할 수 없습니다.", response.getBody().getError().getMessage());
  }
  ...
  void handlesUnexpectedFailure() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handleUnexpected(new IllegalStateException("internal"));
  ...
  void handlesAuthenticationAndUserErrors() {
    assertError(
        handler.handleAuthenticationRequired(new AuthenticationRequiredException()),
        HttpStatus.UNAUTHORIZED,
        "AUTHENTICATION_REQUIRED");
    assertError(
        handler.handleInvalidToken(new GoogleAuthorizationCodeException()),
        HttpStatus.UNAUTHORIZED,
        "INVALID_TOKEN");
    assertError(
        handler.handleInvalidToken(new InvalidOpaqueTokenException()),
        HttpStatus.UNAUTHORIZED,
        "INVALID_TOKEN");
    assertError(
        handler.handleUserNotFound(new UserNotFoundException()),
  ...
  void handlesInvalidHomeQuery() {
    assertError(
        handler.handleInvalidHomeQuery(new InvalidHomeQueryException("invalid")),
        HttpStatus.BAD_REQUEST,
        "INVALID_HOME_QUERY");
  }

  @Test
  @DisplayName("혜택 이력과 리포트의 조회 조건 오류를 각 계약 코드로 변환한다")
  void handlesBenefitHistoryAndReportQueryErrors() {
    assertError(
        handler.handleInvalidBenefitHistoryQuery(
            new InvalidBenefitHistoryQueryException("invalid")),
        HttpStatus.BAD_REQUEST,
    ...
    assertError(
        handler.handleInvalidReportQuery(new InvalidReportQueryException("invalid")),
        HttpStatus.BAD_REQUEST,
  ...
  void handlesHomeDataNotFound() {
    assertError(
        handler.handleHomeDataNotFound(new HomeDataNotFoundException("empty")),
        HttpStatus.NOT_FOUND,
        "HOME_DATA_NOT_FOUND");
  }
  ...
  void handlesIssuerNotFound() {
    ...
    ResponseEntity<ApiErrorResponse> response =
        handler.handleIssuerNotFound(new IssuerNotFoundException(issuerId));

    assertError(response, HttpStatus.NOT_FOUND, "ISSUER_NOT_FOUND");
    assertEquals("등록되지 않은 발급사입니다: " + issuerId, response.getBody().getError().getMessage());
  }
  ...
  void handlesCodefInvalidCredentials() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handleCodefInvalidCredentials(new CodefInvalidCredentialsException());

    assertError(response, HttpStatus.BAD_REQUEST, "CODEF_INVALID_CREDENTIALS");
    assertEquals("아이디 또는 비밀번호가 올바르지 않습니다.", response.getBody().getError().getMessage());
  }
  ...
  void handlesCodefAccountLocked() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handleCodefAccountLocked(new CodefAccountLockedException());

    assertError(response, HttpStatus.LOCKED, "CODEF_ACCOUNT_LOCKED");
    assertEquals(
        "비밀번호 오류 횟수를 초과해 카드사 계정이 잠겼습니다. 카드사를 통해 잠금을 해제한 뒤 다시 시도해주세요.",
  ...
  void handlesCodefConnectionNotFound() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handleCodefConnectionNotFound(new CodefConnectionNotFoundException("0301"));

    assertError(response, HttpStatus.NOT_FOUND, "CODEF_CONNECTION_NOT_FOUND");
    assertEquals("연동된 카드사 계정을 찾을 수 없습니다: 0301", response.getBody().getError().getMessage());
  }
  ...
  void handlesCodefUnavailable() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handleCodefUnavailable(
            new CodefUnavailableException(
                "CODEF 응답이 지연되어 처리하지 못했습니다. 잠시 후 다시 시도해주세요.",
                new java.net.http.HttpTimeoutException("timeout")));

    assertError(response, HttpStatus.SERVICE_UNAVAILABLE, "CODEF_UNAVAILABLE");
    assertEquals(
        "CODEF 응답이 지연되어 처리하지 못했습니다. 잠시 후 다시 시도해주세요.", response.getBody().getError().getMessage());
  }
  ...
  void handlesApprovalSyncFailed() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handleApprovalSyncFailed(
            new ApprovalSyncFailedException(
                "승인내역 동기화에 실패했습니다.", new CodefUnavailableException("CODEF 승인내역 조회에 실패했습니다.")));
  ...
  void handlesPerformanceSyncFailed() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handlePerformanceSyncFailed(
            new PerformanceSyncFailedException(
                "실적조회 동기화에 실패했습니다(issuerId=issuer-1). 잠시 후 다시 시도해주세요."));
  ...
  void handlesPerformanceUnsupported() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handlePerformanceUnsupported(
  ...
  void handlesInvalidSyncPeriod() {
    ResponseEntity<ApiErrorResponse> response =
        handler.handleInvalidSyncPeriod(new InvalidSyncPeriodException("조회 시작일이 종료일보다 늦을 수 없습니다."));

    assertError(response, HttpStatus.BAD_REQUEST, "INVALID_SYNC_PERIOD");
    assertEquals("조회 시작일이 종료일보다 늦을 수 없습니다.", response.getBody().getError().getMessage());
  }
  ...
  void handlesCodefCredentialErrors() {
    ...
    ResponseEntity<ApiErrorResponse> requiredResponse =
        handler.handleCodefCredentialRequired(new CodefCredentialRequiredException(fields));
    ResponseEntity<ApiErrorResponse> duplicateResponse =
        handler.handleCodefAccountAlreadyLinked(new CodefAccountAlreadyLinkedException());
    ResponseEntity<ApiErrorResponse> duplicateCardResponse =
        handler.handleCardAlreadyLinked(
  ...
  void handlesCardCredentialErrors() {
    ...
    ResponseEntity<ApiErrorResponse> requiredResponse =
        handler.handleCardCredentialRequired(new CardCredentialRequiredException(fields));
    ResponseEntity<ApiErrorResponse> mismatchResponse =
        handler.handleCardNumberMismatch(new CardNumberMismatchException());
    ResponseEntity<ApiErrorResponse> notFoundResponse =
        handler.handleUserCardNotFound(new UserCardNotFoundException());
    ...
    assertError(
        handler.handleTypeMismatch(
            new MethodArgumentTypeMismatchException(
                "wrong", Integer.class, "page", methodParameter(), new IllegalArgumentException())),
        HttpStatus.BAD_REQUEST,
        "INVALID_PARAMETER");
  }
  ...
  void handlesHttpStatusErrors() throws Exception {
    ...
    assertError(
        handler.handleMediaTypeNotAcceptable(new HttpMediaTypeNotAcceptableException("invalid")),
        HttpStatus.NOT_ACCEPTABLE,
        "NOT_ACCEPTABLE");
    assertError(
        handler.handleNoHandler(new NoHandlerFoundException("GET", "/missing", new HttpHeaders())),
        HttpStatus.NOT_FOUND,
        "RESOURCE_NOT_FOUND");
  }
