...
@DisplayName("신용 4위 삼성 iD SELECT ALL 카드")
class CreditRank004Card2885BenefitTest {
    ...
    @Test
    @DisplayName("할인: 0.7000000000000001% 할인 정상 적용")
    ...
    @Test
    @DisplayName("할인: 7.000000000000001% 할인 정상 적용")
    ...
    void rejectsBenefit008WhenMerchantIsNotEligible() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit008Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("생활: 지정 결제 채널이 아니면 적용하지 않는다")
    ...
    void rejectsBenefit010WhenCategoryIsNotMatched() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit010Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("해외: 전월 실적 400,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("해외: 전월 실적 400,000원부터 적용한다")
    ...
    void floorsBenefit010FractionalReward() {
        // 비율 계산 결과에 원 미만 소수가 생기는 결제금액으로 절사 정책을 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit010Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
