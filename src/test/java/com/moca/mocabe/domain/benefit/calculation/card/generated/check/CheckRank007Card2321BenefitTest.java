...
@DisplayName("체크 7위 모빌카드")
class CheckRank007Card2321BenefitTest {
    ...
    @Test
    @DisplayName("모든가맹점: 제외 가맹점에서는 적용하지 않는다")
    ...
    @Test
    @DisplayName("모든가맹점: 지정 결제 채널이 아니면 적용하지 않는다")
    ...
    @Test
    @DisplayName("모든가맹점: 정률 혜택의 원 미만 금액을 절사한다")
    ...
    @Test
    @DisplayName("카페/디저트: 7.5% 포인트 정상 적용")
    void appliesBenefit004() {
        // 테스트에 사용한 계산 규칙이 카드고릴라의 해당 혜택 상세에서 만들어졌는지 확인한다.
        fixture.assertSourceDetail(
                /* 카드고릴라 카드 ID */ "2321",
                /* 카드 내 혜택 순번 */ 4,
                /* benefit_title */ "카페/디저트",
                /* 계산 지원 상태 */ "DIRECT_OFFLINE_CALCULABLE",
                /* 분류 사유 */ "직접 카드 결제 혜택 산식 계산 가능");
    ...
    @Test
    @DisplayName("카페/디저트: 카테고리가 일치하지 않으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("카페/디저트: 정률 혜택의 원 미만 금액을 절사한다")
    void floorsBenefit004FractionalReward() {
        ...
        // 카드 혜택은 원 미만 금액을 올림하지 않고 버린 결과와 같아야 한다.
        fixture.assertApplied(result);
        fixture.assertBigDecimalEquals("750", result.rawRewardValue());
    }

    @Test
    @DisplayName("생활: 7.5% 포인트 정상 적용")
    ...
    void floorsBenefit006FractionalReward() {
        ...
        // 카드 혜택은 원 미만 금액을 올림하지 않고 버린 결과와 같아야 한다.
        fixture.assertApplied(result);
        fixture.assertBigDecimalEquals("650", result.rawRewardValue());
    }

    @Test
    @DisplayName("푸드: 10% 할인 정상 적용")
    ...
    @Test
    @DisplayName("교통: 정보성 혜택 - 바우처·수수료·무이자할부 등 결제금액 역산 대상이 아닌 혜택")
    ...
    /**
     * benefit_title=모든가맹점의 계산 조건을 만든다.
     *
    ...
    /**
     * benefit_title=카페/디저트의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
