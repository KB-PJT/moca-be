...
@DisplayName("체크 83위 현대카드M CHECK-경차전용카드(유류세 환급)")
class CheckRank083Card2197BenefitTest {
    ...
    @Test
    @DisplayName("멤버십포인트: 100% 마일리지 정상 적용")
    ...
    @Test
    @DisplayName("멤버십포인트: 카테고리가 일치하지 않으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("멤버십포인트: 제외 가맹점에서는 적용하지 않는다")
    ...
    @Test
    @DisplayName("멤버십포인트: 정률 혜택의 원 미만 금액을 절사한다")
    ...
    /**
     * benefit_title=멤버십포인트의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
