...
class CardGorillaSeedIntegrationTest {
    ...
    void seedsCardGorillaCatalogIdempotently() {
        try (MySQLContainer container = new MySQLContainer(DockerImageName.parse("mysql:8.0.36"))
            ...
            assertEquals(200, count(jdbcTemplate,
                    "SELECT COUNT(*) FROM cards WHERE gorilla_card_id IS NOT NULL"));
            ...
            assertEquals(EXISTING_CARD_ID, jdbcTemplate.queryForObject(
                    "SELECT card_id FROM cards WHERE gorilla_card_id = '13'", String.class));
            assertEquals("신한카드 Mr.Life", jdbcTemplate.queryForObject(
            ...
            assertEquals(3, count(jdbcTemplate,
                    "SELECT COUNT(*) FROM card_performance_tiers tier "
                            + "INNER JOIN card_content_versions cv ON cv.content_version_id = tier.content_version_id "
                            + "INNER JOIN cards c ON c.card_id = cv.card_id "
                            + "WHERE c.gorilla_card_id = '13'"));
            assertEquals(499999, jdbcTemplate.queryForObject(
                    "SELECT maximum_spend_krw FROM card_performance_tiers tier "
                            + "INNER JOIN card_content_versions cv ON cv.content_version_id = tier.content_version_id "
                            + "INNER JOIN cards c ON c.card_id = cv.card_id "
                            + "WHERE c.gorilla_card_id = '13' AND tier.tier_number = 1",
                    Integer.class));
        }
    }
    ...
    private void insertExistingMrLife(JdbcTemplate jdbcTemplate) {
        ...
        jdbcTemplate.update("INSERT INTO cards "
                ...
                EXISTING_CARD_ID, EXISTING_ISSUER_ID);
        jdbcTemplate.update("INSERT INTO card_content_versions "
                ...
                EXISTING_CONTENT_ID, EXISTING_CARD_ID,
