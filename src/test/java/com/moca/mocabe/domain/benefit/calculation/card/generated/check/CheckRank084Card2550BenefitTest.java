...
@DisplayName("체크 84위 노리2 체크카드(Play)")
class CheckRank084Card2550BenefitTest {
    ...
    void rejectsBenefit001WhenPaymentChannelIsNotEligible() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit001Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("게임: 온라인·간편결제 범위 제외 - 온라인·앱·배달·간편결제·자동납부 전용 혜택")
    ...
    @Test
    @DisplayName("공연/전시: 카테고리가 일치하지 않으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("공연/전시: 전월 실적 200,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("공연/전시: 전월 실적 200,000원부터 적용한다")
    ...
    @Test
    @DisplayName("공연/전시: 정률 혜택의 원 미만 금액을 절사한다")
    ...
    @Test
    @DisplayName("헤어: 카테고리가 일치하지 않으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("헤어: 전월 실적 200,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("헤어: 전월 실적 200,000원부터 적용한다")
    ...
    @Test
    @DisplayName("헤어: 정률 혜택의 원 미만 금액을 절사한다")
    ...
    @Test
    @DisplayName("테마파크: 직접 카드 결제 계산 규칙 검토 필요 - 상세 조건에서 완전한 계산 산식을 추출하지 못함")
    ...
    @Test
    @DisplayName("게임: 10% 할인 정상 적용")
    void appliesBenefit011() {
        // 테스트에 사용한 계산 규칙이 카드고릴라의 해당 혜택 상세에서 만들어졌는지 확인한다.
        fixture.assertSourceDetail(
                /* 카드고릴라 카드 ID */ "2550",
                /* 카드 내 혜택 순번 */ 11,
        ...
        // 카드 혜택 룰과 현재 결제 상황을 조합해 예상 혜택을 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit011Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
                        /* 결제금액 */ "100000",
                        /* 리터·횟수 등 사용량 */ "0",
                        /* 전월 실적 */ "10000000",
    ...
    @Test
    @DisplayName("게임: 전월 실적 300,000원보다 1원 적으면 적용하지 않는다")
    void rejectsBenefit011WhenPerformanceIsOneWonBelowRequirement() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit011Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("게임: 전월 실적 300,000원부터 적용한다")
    void appliesBenefit011WhenPerformanceEqualsRequirement() {
        // 적용 가능한 마지막 경계값 또는 최초 경계값을 결제 상황에 넣어 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit011Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("게임: 정률 혜택의 원 미만 금액을 절사한다")
    void floorsBenefit011FractionalReward() {
        // 비율 계산 결과에 원 미만 소수가 생기는 결제금액으로 절사 정책을 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit011Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("패스트푸드: 직접 카드 결제 계산 규칙 검토 필요 - 상세 조건에서 완전한 계산 산식을 추출하지 못함")
    ...
    /**
     * benefit_title=공연/전시의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
    ...
    /**
     * benefit_title=게임의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
    private BenefitRule benefit011Rule(String usedMonthlyValue) {
        return fixture.rule(
                /* 룰 식별자 */ "card-2550-benefit-11",
                /* 할인·캐시백·포인트·마일리지 구분 */ BenefitType.DISCOUNT,
                /* 정률·정액·결제 단위·사용량 단위 구분 */ BenefitBasis.RATE,
                /* 혜택 결과 단위 */ RewardUnit.KRW,
                /* 정률 계산 비율 */ "0.1",
