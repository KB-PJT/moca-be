...
@DisplayName("체크 93위 스타체크카드")
class CheckRank093Card351BenefitTest {
    ...
    @Test
    @DisplayName("주유소: 사용 단위당 50원 할인 정상 적용")
    ...
    @Test
    @DisplayName("주유소: 전월 실적 200,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("주유소: 전월 실적 200,000원부터 적용한다")
    ...
    @Test
    @DisplayName("주유소: 신규 발급 유예기간에는 전월 실적 0원도 적용한다")
    ...
    @Test
    @DisplayName("주유소: 금요일 23시 59분 59초에는 주말 혜택을 적용하지 않는다")
    void rejectsBenefit001WhenBeforeWeekend() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit001Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("주유소: 토요일 00시 정각부터 주말 혜택을 적용한다")
    void appliesBenefit001WhenAtWeekendStart() {
        // 적용 가능한 마지막 경계값 또는 최초 경계값을 결제 상황에 넣어 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit001Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("주유소: 일요일 23시 59분 59초까지 주말 혜택을 적용한다")
    void appliesBenefit001WhenBeforeWeekendEnd() {
        // 적용 가능한 마지막 경계값 또는 최초 경계값을 결제 상황에 넣어 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit001Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("주유소: 월요일 00시 정각부터 주말 혜택을 적용하지 않는다")
    void rejectsBenefit001WhenAtWeekendEnd() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit001Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("주유소: 사용량이 0이면 혜택도 0으로 계산한다")
    void calculatesZeroBenefit001ForZeroUsage() {
    ...
    @Test
    @DisplayName("영화: 직접 카드 결제 계산 규칙 검토 필요 - 상세 조건에서 완전한 계산 산식을 추출하지 못함")
    ...
    @Test
    @DisplayName("드럭스토어: 카테고리가 일치하지 않으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("드럭스토어: 최소 결제금액 30,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("드럭스토어: 최소 결제금액 30,000원부터 적용한다")
    ...
    @Test
    @DisplayName("드럭스토어: 전월 실적 200,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("드럭스토어: 전월 실적 200,000원부터 적용한다")
    ...
    @Test
    @DisplayName("드럭스토어: 신규 발급 유예기간에는 전월 실적 0원도 적용한다")
    ...
    @Test
    @DisplayName("드럭스토어: 1회 인정금액 50,000원 초과분은 혜택에서 제외한다")
    ...
    @Test
    @DisplayName("드럭스토어: 제외 가맹점에서는 적용하지 않는다")
    ...
    @Test
    @DisplayName("드럭스토어: 지정 결제 채널이 아니면 적용하지 않는다")
    ...
    @Test
    @DisplayName("드럭스토어: 정률 혜택의 원 미만 금액을 절사한다")
    void floorsBenefit007FractionalReward() {
        ...
        // 카드 혜택은 원 미만 금액을 올림하지 않고 버린 결과와 같아야 한다.
        fixture.assertApplied(result);
        fixture.assertBigDecimalEquals("1500", result.rawRewardValue());
    }

    @Test
    @DisplayName("도서: 3% 할인 정상 적용")
    ...
    void floorsBenefit008FractionalReward() {
        ...
        // 카드 혜택은 원 미만 금액을 올림하지 않고 버린 결과와 같아야 한다.
        fixture.assertApplied(result);
        fixture.assertBigDecimalEquals("600", result.rawRewardValue());
    }

    @Test
    @DisplayName("패밀리레스토랑: 10% 할인 정상 적용")
    void appliesBenefit009() {
        // 테스트에 사용한 계산 규칙이 카드고릴라의 해당 혜택 상세에서 만들어졌는지 확인한다.
        fixture.assertSourceDetail(
                /* 카드고릴라 카드 ID */ "351",
                /* 카드 내 혜택 순번 */ 9,
                /* benefit_title */ "패밀리레스토랑",
                /* 계산 지원 상태 */ "DIRECT_OFFLINE_CALCULABLE",
                /* 분류 사유 */ "직접 카드 결제 혜택 산식 계산 가능");
    ...
    @Test
    @DisplayName("패밀리레스토랑: 최소 결제금액 30,000원보다 1원 적으면 적용하지 않는다")
    void rejectsBenefit009WhenPaymentIsOneWonBelowMinimum() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit009Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("패밀리레스토랑: 최소 결제금액 30,000원부터 적용한다")
    void appliesBenefit009WhenPaymentEqualsMinimum() {
        // 적용 가능한 마지막 경계값 또는 최초 경계값을 결제 상황에 넣어 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit009Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("패밀리레스토랑: 전월 실적 200,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("패밀리레스토랑: 전월 실적 200,000원부터 적용한다")
    ...
    @Test
    @DisplayName("패밀리레스토랑: 신규 발급 유예기간에는 전월 실적 0원도 적용한다")
    void appliesBenefit009WhenNewMemberGracePeriodIsActive() {
        // 적용 가능한 마지막 경계값 또는 최초 경계값을 결제 상황에 넣어 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit009Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("패밀리레스토랑: 제외 가맹점에서는 적용하지 않는다")
    ...
    @Test
    @DisplayName("패밀리레스토랑: 지정 결제 채널이 아니면 적용하지 않는다")
    void rejectsBenefit009WhenPaymentChannelIsNotEligible() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit009Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("패밀리레스토랑: 정률 혜택의 원 미만 금액을 절사한다")
    ...
    @Test
    @DisplayName("기타: 200000원 정액 할인 정상 적용")
    void appliesBenefit010() {
        // 테스트에 사용한 계산 규칙이 카드고릴라의 해당 혜택 상세에서 만들어졌는지 확인한다.
        fixture.assertSourceDetail(
                /* 카드고릴라 카드 ID */ "351",
                /* 카드 내 혜택 순번 */ 10,
        ...
        // 카드 혜택 룰과 현재 결제 상황을 조합해 예상 혜택을 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit010Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
                        /* 결제금액 */ "100000",
                        /* 리터·횟수 등 사용량 */ "0",
                        /* 전월 실적 */ "10000000",
    ...
    @Test
    @DisplayName("기타: 전월 실적 200,000원보다 1원 적으면 적용하지 않는다")
    void rejectsBenefit010WhenPerformanceIsOneWonBelowRequirement() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit010Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("기타: 전월 실적 200,000원부터 적용한다")
    void appliesBenefit010WhenPerformanceEqualsRequirement() {
        // 적용 가능한 마지막 경계값 또는 최초 경계값을 결제 상황에 넣어 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit010Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    void appliesBenefit010WhenNewMemberGracePeriodIsActive() {
        // 적용 가능한 마지막 경계값 또는 최초 경계값을 결제 상황에 넣어 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit010Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    void capsBenefit010FixedDiscountAtPaymentAmount() {
    ...
    private BenefitRule benefit001Rule(String usedMonthlyValue) {
        return fixture.rule(
                /* 룰 식별자 */ "card-351-benefit-1",
                /* 할인·캐시백·포인트·마일리지 구분 */ BenefitType.DISCOUNT,
                /* 정률·정액·결제 단위·사용량 단위 구분 */ BenefitBasis.PER_USAGE_UNIT,
    ...
    /**
     * benefit_title=드럭스토어의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
    ...
    /**
     * benefit_title=패밀리레스토랑의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
     ...
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
    private BenefitRule benefit010Rule(String usedMonthlyValue) {
        return fixture.rule(
                /* 룰 식별자 */ "card-351-benefit-10",
                /* 할인·캐시백·포인트·마일리지 구분 */ BenefitType.DISCOUNT,
                /* 정률·정액·결제 단위·사용량 단위 구분 */ BenefitBasis.FIXED,
                /* 혜택 결과 단위 */ RewardUnit.KRW,
                /* 정률 계산 비율 */ "0",
                /* 정액 또는 단위당 혜택값 */ "200000",
                /* 단위 적립 기준 결제금액 */ "0",
                /* 1회 혜택 인정금액 상한 */ "0",
                /* 혜택 적용 최소 결제금액 */ "0",
                /* 필요한 전월 실적 */ "200000",
