...
@DisplayName("체크 9위 토스뱅크 체크카드")
class CheckRank009Card2269BenefitTest {
    ...
    @Test
    @DisplayName("오프라인 캐시백: 카테고리가 일치하지 않으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("오프라인 캐시백: 월 10회 한도의 마지막 허용 거래는 적용한다")
    ...
    @Test
    @DisplayName("오프라인 캐시백: 월 10회 사용 후에는 적용하지 않는다")
    ...
    @Test
    @DisplayName("오프라인 캐시백: 지정 결제 채널이 아니면 적용하지 않는다")
    ...
    @Test
    @DisplayName("어디서나 캐시백: 정률 혜택의 원 미만 금액을 절사한다")
    ...
    @Test
    @DisplayName("기부 캐시백: 직접 카드 결제 계산 규칙 검토 필요 - 선택형/자동맞춤/배수/특수 조건은 별도 사용자 선택 또는 집계가 필요함")
    void classifiesBenefit004() {
        // 계산하지 않는 상세도 누락하지 않고 원본 분류 상태와 제외·검토 사유를 고정한다.
        fixture.assertSourceDetail(
                /* 카드고릴라 카드 ID */ "2269",
                /* 카드 내 혜택 순번 */ 4,
                /* benefit_title */ "기부 캐시백",
                /* 계산 지원 상태 */ "DIRECT_OFFLINE_REVIEW_REQUIRED",
                /* 분류 사유 */ "선택형/자동맞춤/배수/특수 조건은 별도 사용자 선택 또는 집계가 필요함");
    }

    @Test
    @DisplayName("공통 혜택: 2% 캐시백 정상 적용")
    ...
    @Test
    @DisplayName("공통 혜택: 카테고리가 일치하지 않으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("공통 혜택: 월 5회 한도의 마지막 허용 거래는 적용한다")
    ...
    @Test
    @DisplayName("공통 혜택: 월 5회 사용 후에는 적용하지 않는다")
    ...
    @Test
    @DisplayName("공통 혜택: 정률 혜택의 원 미만 금액을 절사한다")
    ...
    /**
     * benefit_title=오프라인 캐시백의 계산 조건을 만든다.
     *
    ...
    /**
     * benefit_title=어디서나 캐시백의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
    ...
    /**
     * benefit_title=공통 혜택의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
