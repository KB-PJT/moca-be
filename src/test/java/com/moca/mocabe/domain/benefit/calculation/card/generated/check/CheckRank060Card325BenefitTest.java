...
@DisplayName("체크 60위 그랑블루 체크카드")
class CheckRank060Card325BenefitTest {
    ...
    @Test
    @DisplayName("바우처: 직접 카드 결제 계산 규칙 검토 필요 - 상세 조건에서 완전한 계산 산식을 추출하지 못함")
    ...
    @Test
    @DisplayName("여행/숙박: 정보성 혜택 - 바우처·수수료·무이자할부 등 결제금액 역산 대상이 아닌 혜택")
    ...
    @Test
    @DisplayName("푸드: 1회 인정금액 5,000원 초과분은 혜택에서 제외한다")
    ...
    @Test
    @DisplayName("푸드: 월 1회 한도의 마지막 허용 거래는 적용한다")
    ...
    @Test
    @DisplayName("푸드: 월 1회 사용 후에는 적용하지 않는다")
    ...
    @Test
    @DisplayName("레저/스포츠: 40% 할인 정상 적용")
    void appliesBenefit007() {
        // 테스트에 사용한 계산 규칙이 카드고릴라의 해당 혜택 상세에서 만들어졌는지 확인한다.
        fixture.assertSourceDetail(
                /* 카드고릴라 카드 ID */ "325",
                /* 카드 내 혜택 순번 */ 7,
                /* benefit_title */ "레저/스포츠",
                /* 계산 지원 상태 */ "DIRECT_OFFLINE_CALCULABLE",
                /* 분류 사유 */ "직접 카드 결제 혜택 산식 계산 가능");
    ...
    @Test
    @DisplayName("레저/스포츠: 전월 실적 300,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("레저/스포츠: 전월 실적 300,000원부터 적용한다")
    ...
    @Test
    @DisplayName("레저/스포츠: 월 1회 한도의 마지막 허용 거래는 적용한다")
    ...
    @Test
    @DisplayName("레저/스포츠: 월 1회 사용 후에는 적용하지 않는다")
    ...
    @Test
    @DisplayName("레저/스포츠: 제외 가맹점에서는 적용하지 않는다")
    ...
    @Test
    @DisplayName("레저/스포츠: 정률 혜택의 원 미만 금액을 절사한다")
    ...
    void calculatesZeroBenefit008ForZeroUsage() {
    ...
    /**
     * benefit_title=레저/스포츠의 계산 조건을 만든다.
     *
     * @param usedMonthlyValue 이번 달에 이미 적용받은 혜택 누적값
     * @return 금액·실적·한도·횟수·적용 조건이 구조화된 혜택 룰
     */
