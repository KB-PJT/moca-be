...
@DisplayName("신용 77위 BC 바로 Air Max 카드")
class CreditRank077Card2927BenefitTest {
    ...
    @Test
    @DisplayName("대한항공: 1,500원당 1마일 적립 정상 적용")
    ...
    @Test
    @DisplayName("대한항공: 카테고리가 일치하지 않으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("대한항공: 다음 결제 단위보다 1원 적은 금액은 추가 적립하지 않는다")
    void excludesBenefit004AmountBelowNextSpendUnit() {
        // 다음 적립 단위에 1원 모자란 결제금액으로 불완전한 단위가 버려지는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit004Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
    ...
    @Test
    @DisplayName("대한항공: 다음 결제 단위 경계값부터 추가 혜택을 적용한다")
    void appliesBenefit004AtNextSpendUnit() {
        // 다음 적립 단위와 정확히 같은 결제금액으로 추가 혜택 시작 경계를 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit004Rule(/* 이번 달에 이미 사용한 혜택 */ "0"),
                fixture.context(
