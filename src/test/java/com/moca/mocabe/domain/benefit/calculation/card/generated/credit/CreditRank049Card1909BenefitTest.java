...
@DisplayName("신용 49위 THE 1 (스카이패스)")
class CreditRank049Card1909BenefitTest {
    ...
    @Test
    @DisplayName("바우처: 직접 카드 결제 계산 규칙 검토 필요 - 선택형/자동맞춤/배수/특수 조건은 별도 사용자 선택 또는 집계가 필요함")
    ...
    @Test
    @DisplayName("대한항공: 1,000원당 2마일 적립 정상 적용")
    ...
    @Test
    @DisplayName("대한항공: 전월 실적 300,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("대한항공: 전월 실적 300,000원부터 적용한다")
    ...
    @Test
    @DisplayName("대한항공: 제외 가맹점에서는 적용하지 않는다")
    ...
    @Test
    @DisplayName("대한항공: 지정 결제 채널이 아니면 적용하지 않는다")
    ...
    @Test
    @DisplayName("대한항공: 다음 결제 단위보다 1원 적은 금액은 추가 적립하지 않는다")
    void excludesBenefit002AmountBelowNextSpendUnit() {
    ...
    @Test
    @DisplayName("대한항공: 다음 결제 단위 경계값부터 추가 혜택을 적용한다")
    void appliesBenefit002AtNextSpendUnit() {
        ...
        fixture.assertApplied(result);
        fixture.assertBigDecimalEquals("2", result.rawRewardValue());
    }

    @Test
    @DisplayName("택시: 10000마일 정액 마일리지 정상 적용")
    ...
    @Test
    @DisplayName("택시: 전월 실적 300,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("택시: 전월 실적 300,000원부터 적용한다")
    ...
    @Test
    @DisplayName("택시: 월 1회 한도의 마지막 허용 거래는 적용한다")
    ...
    @Test
    @DisplayName("택시: 월 1회 사용 후에는 적용하지 않는다")
    ...
    @Test
    @DisplayName("카페: 10000마일 정액 마일리지 정상 적용")
    ...
    @Test
    @DisplayName("공항: 30000마일 정액 마일리지 정상 적용")
    ...
    @Test
    @DisplayName("공항: 전월 실적 300,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("공항: 전월 실적 300,000원부터 적용한다")
    ...
    @Test
    @DisplayName("공항: 월 1회 한도의 마지막 허용 거래는 적용한다")
    ...
    @Test
    @DisplayName("공항: 월 1회 사용 후에는 적용하지 않는다")
    ...
    @Test
    @DisplayName("프리미엄 서비스: 정보성 혜택 - 바우처·수수료·무이자할부 등 결제금액 역산 대상이 아닌 혜택")
    void classifiesBenefit006() {
        // 계산하지 않는 상세도 누락하지 않고 원본 분류 상태와 제외·검토 사유를 고정한다.
        fixture.assertSourceDetail(
                /* 카드고릴라 카드 ID */ "1909",
                /* 카드 내 혜택 순번 */ 6,
                /* benefit_title */ "프리미엄 서비스",
    ...
    /**
     * benefit_title=대한항공의 계산 조건을 만든다.
     *
    ...
    private BenefitRule benefit002Rule(String usedMonthlyValue) {
        return fixture.rule(
                /* 룰 식별자 */ "card-1909-benefit-2",
                /* 할인·캐시백·포인트·마일리지 구분 */ BenefitType.MILEAGE,
                /* 정률·정액·결제 단위·사용량 단위 구분 */ BenefitBasis.PER_SPEND_UNIT,
                /* 혜택 결과 단위 */ RewardUnit.MILE,
                /* 정률 계산 비율 */ "0",
                /* 정액 또는 단위당 혜택값 */ "2",
                /* 단위 적립 기준 결제금액 */ "1000",
                /* 1회 혜택 인정금액 상한 */ "0",
