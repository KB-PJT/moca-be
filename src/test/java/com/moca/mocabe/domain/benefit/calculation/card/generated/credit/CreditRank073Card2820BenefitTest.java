...
@DisplayName("신용 73위 원더카드 2.0 FREE+")
class CreditRank073Card2820BenefitTest {
    ...
    @Test
    @DisplayName("국내외가맹점: 0.8% 할인 정상 적용")
    ...
    @Test
    @DisplayName("국내외가맹점: 전월 실적 400,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("국내외가맹점: 전월 실적 400,000원부터 적용한다")
    ...
    void floorsBenefit001FractionalReward() {
        ...
        fixture.assertApplied(result);
        fixture.assertBigDecimalEquals("80", result.rawRewardValue());
    }
    ...
    @Test
    @DisplayName("쇼핑: 온라인·간편결제 범위 제외 - 온라인·앱·배달·간편결제·자동납부 전용 혜택")
    ...
    @Test
    @DisplayName("택시: 전월 실적 400,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("택시: 전월 실적 400,000원부터 적용한다")
    ...
    @Test
    @DisplayName("택시: 월 한도가 1원 남았으면 1원만 적용한다")
    void appliesOnlyRemainingMonthlyLimitForBenefit007() {
        // 월 한도에서 이미 사용한 혜택을 빼고 1원만 남은 상태로 계산한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit007Rule(/* 이번 달에 이미 사용한 혜택 */ "799999"),
                fixture.context(
                        /* 결제금액 */ "100000",
                        /* 리터·횟수 등 사용량 */ "0",
                        /* 전월 실적 */ "10000000",
    ...
    @Test
    @DisplayName("택시: 월 한도를 모두 사용했으면 적용하지 않는다")
    void rejectsBenefit007WhenMonthlyLimitIsExhausted() {
        // 미적용 경계값을 결제 상황에 넣어 계산기가 거절 사유를 구분하는지 확인한다.
        BenefitCalculationResult result = fixture.calculator.calculate(
                benefit007Rule(/* 이번 달에 이미 사용한 혜택 */ "800000"),
                fixture.context(
                        /* 결제금액 */ "100000",
                        /* 리터·횟수 등 사용량 */ "0",
                        /* 전월 실적 */ "10000000",
    ...
    void floorsBenefit007FractionalReward() {
        ...
        // 카드 혜택은 원 미만 금액을 올림하지 않고 버린 결과와 같아야 한다.
        fixture.assertApplied(result);
        fixture.assertBigDecimalEquals("400", result.rawRewardValue());
    }

    @Test
    @DisplayName("무이자할부: 50000P 정액 포인트 정상 적용")
    ...
    @Test
    @DisplayName("무이자할부: 전월 실적 400,000원보다 1원 적으면 적용하지 않는다")
    ...
    @Test
    @DisplayName("무이자할부: 전월 실적 400,000원부터 적용한다")
