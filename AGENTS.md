...
3. 새 기능은 `feature/<feature-name>`, 수정 작업은 `fix/<fix-name>` 또는 `hotfix/<hotfix-name>`, 문서 작업은 `docs/<topic>`, 운영·개발 환경 정리는 `chore/<topic>` 브랜치를 최신 `main`에서 만든 후 작업한다.
4. 현재 브랜치의 부모 이슈·작업 목적과 다른 새 변경은 현재 브랜치나 `main`에서 바로 시작하지 않는다. 최신 `main`을 동기화한 뒤 변경 유형에 맞는 새 브랜치를 만들고 그 브랜치에서만 작업한다.
5. `main`에 기능을 직접 커밋하지 않는다.
